...
use crate::simulation::ShallowWaterParams;
...
pub struct GpuShallowWaterBackend {
    ...
    /// Ping-pong field buffers: vec4<f32> per cell (h, vx, vy, _)
    field_a: wgpu::Buffer,
    field_b: wgpu::Buffer,
...
impl GpuShallowWaterBackend {
    pub fn new(display: &Display, width: u32, height: u32, params: &ShallowWaterParams) -> Self {
    ...
    fn build(
        ...
        let source = include_str!("shallow_water.wgsl");
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("shallow-water compute"),
        ...
        let uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("shallow-water params"),
        ...
        let field_size = n * 16; // vec4<f32> = 16 bytes per cell
        ...
        // Upload initial state: h=1.0, vx=0, vy=0 with seeded bumps
        let mut init_data = vec![[1.0f32, 0.0f32, 0.0f32, 0.0f32]; n as usize];
        ...
        for _ in 0..5 {
            ...
            for dy in -(radius as isize)..=(radius as isize) {
                for dx in -(radius as isize)..=(radius as isize) {
                    ...
                    init_data[y * width as usize + x][0] += 0.3 * t * t;
                }
            }
        }
    ...
    /// Inject a height bump at pixel position (for mouse interaction).
    pub fn inject(&self, queue: &wgpu::Queue, px: u32, py: u32, radius: u32) {
        ...
        for dy in -r..=r {
            for dx in -r..=r {
                ...
                let t = 1.0 - (d2 as f32 / r2 as f32);
                // Read-modify-write is expensive on GPU; just write a bump height directly.
                // This overwrites vx/vy to 0, which is fine — the bump propagates naturally.
                let data: [f32; 4] = [1.0 + 0.15 * t * t, 0.0, 0.0, 0.0];
