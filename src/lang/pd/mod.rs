...
mod tests {
    ...
    fn parse_gradient_pd() {
        let src = std::fs::read_to_string("examples/basic/gradient.pd").unwrap();
    ...
    fn parse_mandelbrot_pd() {
        let src = std::fs::read_to_string("examples/basic/mandelbrot.pd").unwrap();
        let kernel = parse(&src).unwrap();
    ...
    fn parse_sdf_flower_pd() {
        let src = std::fs::read_to_string("examples/sdf/sdf_flower.pd").unwrap();
        let kernel = parse(&src).unwrap();
    ...
    fn parse_cornell_2d_pd() {
        let src = std::fs::read_to_string("examples/lighting/cornell_2d.pd").unwrap();
        let kernel = parse(&src).unwrap();
