...
mod tests {
    ...
    fn parse_gradient_pd() {
        let path = "examples/basic/gradient.pd";
    ...
    fn parse_mandelbrot_pd() {
        let kernel = parse_file("examples/basic/mandelbrot.pd");
    ...
    fn parse_sdf_flower_pd() {
        let kernel = parse_file("examples/sdf/sdf_flower.pd");
        assert_eq!(kernel.name, "sdf_flower");
    }

    #[test]
    fn parse_cornell_2d_pd() {
        let kernel = parse_file("examples/lighting/cornell_2d.pd");
    ...
    fn gradient_pd_produces_valid_ir() {
        let kernel = parse_file("examples/basic/gradient.pd");
