...
The JIT tile loop (not the host, not the kernel) computes jitter per pixel:
...
This keeps kernels simple. For Level 2/3, kernel-side RNG ops will be added so the kernel can generate random ray directions internally.
...
Initially: SDF scenes are hardcoded in WGSL kernels. The kernel receives `(x, y)` and evaluates its built-in scene.
