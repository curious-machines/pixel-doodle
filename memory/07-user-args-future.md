...
WGSL kernels receive built-in values (coordinates, dimensions, time) automatically via the tile loop or uniform buffer. User-defined args are passed separately. The `.pdp` `run` statement only passes user-defined args — built-in values are injected automatically.

This is slightly inconsistent: the pdp author only passes some parameters. The user noted this bothers them.
